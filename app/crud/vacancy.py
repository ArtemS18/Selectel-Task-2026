...
async def upsert_external_vacancies(
    ...
    if external_ids:
        existing_result = await session.execute(
            select(Vacancy).where(Vacancy.external_id.in_(external_ids)) # сразу получу все чтобы потом не делать запросы по одному при обновлении
        )
        existing_ids = {vacancy.external_id: vacancy for vacancy in existing_result.scalars().all()}
    else:
        existing_ids = {}
    ...
    for payload in payloads:
        ...
        if ext_id and (ext_id in existing_ids):
            vacancy = existing_ids[ext_id] # иду в словарь по external_id и получаю объект без запроса к бд
